...
function checkSSH {
    ...
    CHECK_PORT_NUM=$(grep -c "^Port " /etc/ssh/sshd_config)
    if [ "${CHECK_PORT_NUM}" -gt "5" ] || [ -z "${CHECK_SSH_OPEN}" ] ; then
        ...
        for numb in $(ufw status numbered | awk -F"[" '{print $NF}' | awk '{print $1}' | cut -d ']' -f1 | tail -n+5 | sort -rn); do
            echo y | ufw delete "$numb"
        done
        sudo ufw allow 22
        sudo ufw allow 53
...
function openPorts {
    ...
    # Fix #9: properly quote all variables
    ...
    while read -r line; do
        ...
        if [ "${ipCount}" -gt "50" ]; then
            ...
            while read -r port; do
                echo "[x] Port attempt on ${port}"
                echo "[+] Allowing SSH on port ${port}"
                checkPort=$(grep "Port ${port}" /etc/ssh/sshd_config)

                if [ -z "${checkPort}" ]; then
                    echo "port empty ${port}"
                    sed -i.bak $"s/Port 22/Port 22\\nPort ${port}/g" /etc/ssh/sshd_config
                    sudo service sshd restart
                fi

                echo "[+] Allow port ${port} from ${ipAddress}"
