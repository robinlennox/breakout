...
def multiprocessing(port_list, aggressive, verbose, scan_type, threadcount):
    pool = ThreadPool(threadcount)
    pool.map(partial(scan_type, aggressive=aggressive, verbose=verbose),
             port_list)
    pool.close()
    pool.join()
...
def check_port(scan_type, aggressive, verbose,):
    ...
    #common_ports = [80, 53, 110, 500, 995, 1723, 443, 21, 110, 123, 143, 264, 587, 993, 465, 6080, 3389, 8001, 8014, 8028, 8081, 8020, 8030, 8023, 8080]
    ...
    try:
        multiprocessing(common_ports, aggressive, verbose, scan_type, 1)
        if openPorts:
            pass
    ...
    if aggressive:
        ...
        multiprocessing(port_list, aggressive, verbose, scan_type, 400)


def traceroute_port_check(portNumber, aggressive, verbose):
    global check_count
    global quitscan
    check_none = 0
...
def portquiz_scan(portNumber, aggressive, verbose):
    global check_count
    global quitscan
    # Stop after finding one port open to prevent detection.
    if quitscan >= 3 and aggressive is False:
        return
    elif quitscan >= 100:
        return
